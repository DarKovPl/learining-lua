...
function play_computer_guesses_game(max_range)
  ...
  function play_mix_game(min_range, max_range, players)
    ...
    for i = 1, #players do
      answer_player[players[i].name] = math.random(min_range, max_range)
      num_guesses_player[players[i].name] = 0
    end
    ...
    while winner == nil do
      for i = 1, #players do
        -- Player's turn
        local guess_player = get_integer_input(players[i].name .. ", guess a number between " .. min_range .. " and " .. max_range .. ": ")
        num_guesses_player[players[i].name] = num_guesses_player[players[i].name] + 1
  
        if guess_player == answer_player[players[i].name] then
          winner = players[i].name
          print(players[i].name .. " wins! You guessed the number in " .. num_guesses_player[players[i].name] .. " tries!")
          break
        elseif guess_player < answer_player[players[i].name] then
          print("The number is higher than your guess.")
        else
          print("The number is lower than your guess.")
        end
...
function main()
    ...
    local num_players = get_integer_input("Enter the number of players: ")
    local players = {}
